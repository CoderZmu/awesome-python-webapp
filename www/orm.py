...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        attrs['__fields__'] = fields  # 除主键外的属性列表
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def __getattr__(self, key):
        try:
            ...
            raise AttributeError(r"'Model' object has no attribuate %s" % key)
    ...
    async def update(self):
        ...
        if rows != 1:
            logging.warn('fail to update by primary key: affected rows: %s' %
                         rows)
    ...
    async def remove(self):
        ...
        if rows != 1:
            logging.warn('fail to remove by primary key: affected rows: %s' %
                         rows)
