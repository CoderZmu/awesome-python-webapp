import asyncio, logging, os, functools, inspect
...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        ...
        if found and (param.kind != inspect.Parameter.VAR_POSITIONAL
            ...
            raise ValueError(
                'request parameter must be the last named parameter in function: %s'
                % (fn.__name__))
    return found
            ...
            if request.method == 'POST':
                ...
                if ct.startswith('application/json'):
                    ...
                    if not isinstance(params, dict):
                        return web.HTTPBadRequest('JSON boyd must be object.')
                    kw = params
...
def add_route(app, fn):
    ...
    if method is None or round is None:
